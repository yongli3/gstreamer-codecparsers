...
parse_tile_info (GstVp9FrameHdr * frame_hdr, GstBitReader * br)
{
  ...
  guint32 max_ones = max_lb_tile_cols - min_lb_tile_cols;
...
segmentation_update (GstVp9Parser * parser, const GstVp9FrameHdr * frame_hdr)
{
  ...
  for (i = 0; i < GST_VP9_MAX_SEGMENTS; i++) {
    ...
    seg->luma_dc_quant_scale = vp9_dc_quant (q, quant_indices->y_dc_delta, frame_hdr->bit_depth);
    seg->luma_ac_quant_scale = vp9_ac_quant (q, 0, frame_hdr->bit_depth);
    seg->chroma_dc_quant_scale = vp9_dc_quant (q, quant_indices->uv_dc_delta, frame_hdr->bit_depth);
    seg->chroma_ac_quant_scale = vp9_ac_quant (q, quant_indices->uv_ac_delta, frame_hdr->bit_depth);
